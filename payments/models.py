...
class Payment(models.Model):
    ...
    paypal_ipn = models.ForeignKey(PayPalIPN, on_delete=models.CASCADE, related_name="payments")
...
class DonationDriveManager(models.Manager):
    def current(self, language_code):
        ...
        return [
            d
            for d in self.filter(start__lte=now, finish__gte=now, language_code=language_code, active=True)
    ...
    def current_for_account(self, account):
        if account.donations_disabled():
            return []
        return [d for d in self.current(account.default_language_code) if d.active_for_account(account)]
...
class DonationDrive(models.Model):
    ...
    def amount_raised_formatted(self):
        return fluent_types.fluent_number(
            self.amount_raised,
    ...
    def get_amount_raised(self, before=None):
        ...
        total1 = initial_qs.filter(
            created_at__gt=self.start, created_at__lt=self.finish, mc_currency=settings.VALID_RECEIVE_CURRENCY
        ).aggregate(models.Sum("mc_gross"))["mc_gross__sum"]
        if total1 is None:
            total1 = Decimal("0")

        total2 = initial_qs.filter(
            created_at__gt=self.start, created_at__lt=self.finish, settle_currency=settings.VALID_RECEIVE_CURRENCY
    ...
    def get_contributions(self):
        return Payment.objects.filter(created__gt=self.start, created__lt=self.finish)
    ...
    def __str__(self):
        return f"{self.start.strftime('%Y-%m-%d')} to {self.finish.strftime('%Y-%m-%d')}"
...
def send_donation_drive_target_reached_emails(donation_drive):
    ...
    recipient_data = [
        {
            ...
            "amount": fluent_types.fluent_number(
                payment.amount,
                style=fluent_types.FORMAT_STYLE_CURRENCY,
                currencyDisplay=fluent_types.CURRENCY_DISPLAY_SYMBOL,
                currency="GBP",
    ...
    c_base = {"target": donation_drive.target_formatted}
