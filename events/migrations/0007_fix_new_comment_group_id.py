...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        NEW_COMMENT = 13
        for event in orm['events.Event'].objects.filter(event_type=NEW_COMMENT):
            c = orm['comments.Comment'].objects.get(id=event.event_data['comment_id'])
            if c.group_id is not None:
                event.event_data['group_id'] = c.group_id
                event.save()
    ...
    models = {
        ...
        u'comments.comment': {
            ...
            'author': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "u'comments'", 'to': u"orm['accounts.Account']"}),
            'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'event': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "u'comments'", 'null': 'True', 'to': u"orm['events.Event']"}),
            'group': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "u'comments'", 'null': 'True', 'to': u"orm['groups.Group']"}),
    ...
    complete_apps = ['comments', 'groups', 'events']
    symmetrical = True
