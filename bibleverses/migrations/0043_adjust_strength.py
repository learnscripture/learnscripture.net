...
from accounts.memorymodel import MemoryModel, MM

class OldMemoryModel(MemoryModel):
    LEARNT = 0.9

OldMM = OldMemoryModel(0.25)
NewMM = MM

class Migration(DataMigration):

    def forwards(self, orm):
        # We changed the memory model, in particular in changing 'LEARNT' from
        # 0.90 to 0.80. We use the old memory model to convert

        for uvs in orm['bibleverses.UserVerseStatus'].objects.all():
            # current 'strength' is calculated according to old model.  Convert
            # it to an absolute 'time', which is in the same in both models.

            if uvs.strength > 0:
                old_strength = uvs.strength
                t = OldMM.t(uvs.strength)
                s = NewMM.s(t)
                uvs.strength = s
                # In order to not confuse users, we leave 'next_test_due'
                # as it was, so until the next test things progress
                # according to the old model.
                uvs.save()


    def backwards(self, orm):
        # above, with OldMM and NewMM switched
        for uvs in orm['bibleverses.UserVerseStatus'].objects.all():
            if uvs.strength > 0:
                old_strength = uvs.strength
                t = NewMM.t(uvs.strength)
                s = OldMM.s(t)
                uvs.strength = s
                uvs.save()
    ...
    models = {
        'accounts.account': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 4, 28, 15, 2, 29, 40241)'}),
            'email': ('django.db.models.fields.EmailField', [], {'unique': 'True', 'max_length': '75'}),
            'first_name': ('django.db.models.fields.CharField', [], {'max_length': '100', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'is_tester': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 4, 28, 15, 2, 29, 40076)'}),
