...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Some UVS's have got further than the memory model should allow.  This
        # is probably due to MemoryModel.strength_estimate incrementing by
        # DELTA_S_IDEAL without limiting according to time_elapsed.

        # It is still confusing how this could have been triggered without
        # deliberate hacking.

        from accounts.memorymodel import MM

        fix_master_awards = set()

        error_start = datetime.datetime(2013, 6, 20, 16, 0, 0)
        i = 0
        for uvs in orm['bibleverses.UserVerseStatus'].objects.filter(last_tested__gte=error_start,
                                                                     first_seen__isnull=False,
                                                                     ):
            s_max = MM.s((uvs.last_tested - uvs.first_seen).total_seconds()
                         + 1000 # A bit extra to allow for the initial strength factor
                         )
            old_strength = uvs.strength
            if old_strength > s_max + 0.00001:

                uvs.strength = s_max
                uvs.next_test_due = MM.next_test_due(uvs.last_tested, uvs.strength)
                uvs.save()

                if old_strength > MM.LEARNT:
                    identity = uvs.for_identity
                    if identity.account is not None:
                        fix_master_awards.add(identity.account.username)
                print i, uvs.id, uvs.reference, uvs.for_identity.account.username, old_strength, uvs.strength
                i += 1
        print fix_master_awards
