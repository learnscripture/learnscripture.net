...
prettyReason reason locale =
    case reason of
        ...
        VerseLearned ->
            T.learnVerseFullyLearnedBonus locale ()
...
viewCurrentVerse session model =
    let
        ...
        verseScaledStrength =
            -- Avoid showing a rounded '100%' for 99.5% and higher
            -- so round down to nearest percent
            toFloat (floor (100 * scaledStrength (currentVerseStrength currentVerse))) / 100
...
{-| see bibleverses/models.py scaled\_strength
-}
scaledStrength : Float -> Float
scaledStrength rawStrength =
    min (rawStrength / MemoryModel.learned) 1.0
...
See learn\_setup.js for why this is necessary.
                                ...
                                ++ (if isPracticeTest verse testType then
                                        ...
                                        case verse.recordedTestScore of
                                            ...
                                            Just recordedTest ->
                                                ...
                                                in
                                                if newStrength >= MemoryModel.learned && verse.verseStatus.strength < MemoryModel.learned then
                                                    [ H.br [] []
...
calculateNextTestDue currentVerse =
    ...
    case currentVerse.recordedTestScore of
        ...
        Just recordedTest ->
            ...
            in
            if newStrength >= MemoryModel.learned then
                Just NeverDue
...
friendlyTimeUntil testdue locale =
    case testdue of
        NeverDue ->
            T.learnVerseFullyLearned locale ()
...
actionLogDecoder =
    JDP.decode ActionLog
        ...
        |> JDP.required "reason" scoreReasonDecoder
...
scoreReasonDecoder : JD.Decoder ScoreReason
scoreReasonDecoder =
    intEnumDecoder
        [ ( 0, VerseTested )
        , ( 1, VerseReviewed )
