...
def session_forms(request):
    ...
    return {
        "preferences_form": lambda: PreferencesForm(
            instance=request.identity if hasattr(request, "identity") else None
        ),
    }
...
def notices(request):
    ...
    if hasattr(request, "identity"):
        ...
        if request.identity.account is not None:
            retval["donation_drives"] = memoize_nullary(
                lambda: DonationDrive.objects.current_for_account(request.identity.account)
            )
...
def request_account(request):
    ...
    if hasattr(request, "identity"):
        ...
        default_language_code = DEFAULT_LANGUAGE.code
    return {"request_account": account, "default_language_code": default_language_code}
