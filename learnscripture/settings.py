...
ADMINS = [("", "admin@learnscripture.net")]
...
if DEBUG or any(a in sys.argv for a in ["setup_bibleverse_suggestions", "run_suggestions_analyzers"]):
    LOGGING["root"] = {
        "level": "WARNING",
        "handlers": ["console"],
    }
        ...
        def filter(self, record):
            if not hasattr(record, "stack_patched"):
                ...
                if hasattr(record, "duration") and hasattr(record, "sql") and hasattr(record, "params"):
                    record.msg = "\nFrom stack: \n\x1b[32m{}\x1b[0m\n \x1b[31mduration: {}{:.4f} secs\x1b[0m, \x1b[33marguments: {}{}\x1b[0m\n  \x1b[1;33m{}\x1b[0m\n".format(
...
if DEBUG:
    ...
    for f in sorted(glob.glob("./learnscripture/static/webpack_bundles/*.dev.*"), reverse=True, key=os.path.getctime)[
        20:
    ]:
        os.unlink(f)
