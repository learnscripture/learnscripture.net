...
class CeleryRedirectedFilter(logging.Filter):
    def filter(self, record):
        if 'Degrade mode - ENTERING' in record.msg:
            # This is a warning generated by ConcurrentRotatingFileHandler (which it
            # prints to stderr, converted to a logging warning by celery). We can
            # safely ignore it, and it is just noise otherwise.
            return False
        return True
