...
class LeaderboardTests(TestBase):
    def setUp(self):
        ...
        a1.add_points(100, ScoreReason.VERSE_FIRST_TESTED)
        a2 = Account.objects.create(username="testuser2", email="test2@test.com", is_active=False)
        Identity.objects.create(account=a2)
        a2.add_points(50, ScoreReason.VERSE_FIRST_TESTED)
        self.a1 = a1
        self.a2 = a2
        group = Group.objects.create(name="My group", slug="my-group", created_by=a1, open=True, public=True)
        group.add_user(a1)
        group.add_user(a2)
        self.group = group
...
class VerseCountTests(BibleVersesMixin, CatechismsMixin, AccountTestMixin, TestBase):
    ...
    def _create_overlapping_verse_sets(self, account):
        vs1 = VerseSet.objects.create(
            name="Psalm 23:1-3", language_code="en", set_type=VerseSetType.PASSAGE, created_by=account
        )
        vs2 = VerseSet.objects.create(
            name="Psalm 23:1-2", language_code="en", set_type=VerseSetType.PASSAGE, created_by=account
        )
        vs1.set_verse_choices(["BOOK18 23:1", "BOOK18 23:2", "BOOK18 23:3"])
        vs2.set_verse_choices(["BOOK18 23:1", "BOOK18 23:2"])
    ...
    def _do_assert_stats(self, identity: Identity, count, refs=None):
        if refs is None:
            ...
            ref_pairs = [
                (r.localized_reference, r.version) for r in identity.verse_statuses.filter(localized_reference__in=refs)
            ]
    ...
    def test_verse_stats_merged(self):
        ...
        identity.record_verse_action("Romalılar 3:25-26", version.slug, StageType.TEST, accuracy=1.0)
    ...
    def test_verse_stats_combos(self):
        ...
        identity.record_verse_action("Psalm 23:1-2", version.slug, StageType.TEST, accuracy=1.0)
    ...
    def test_verse_stats_combos_2(self):
        ...
        identity.record_verse_action("Psalm 23:1", version.slug, StageType.TEST, accuracy=1.0)
        identity.record_verse_action("Psalm 23:2", version.slug, StageType.TEST, accuracy=1.0)
