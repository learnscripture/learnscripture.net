...
class EventTests(AccountTestMixin, TestBase):
    def test_new_comment_event(self):
        ...
        self.assertEqual(event.get_absolute_url(), f"/activity/{orig_event.id}/#comment-{comment.id}")

        self.assertFalse(event.accepts_comments())

        def assert_originator_notification(account, count):
            self.assertEqual(
                len([n for n in account.identity.notices.all() if "You have new" in n.message_html]), count
            )

        def assert_contributor_notification(account, count):
            self.assertEqual(len([n for n in account.identity.notices.all() if "There are" in n.message_html]), count)
    ...
    def test_created_display(self):
        with django_ftl.override("en"):
            ...
            self.assertEqual(event.created_display(), "Just now")
            self.move_clock_on(timedelta(seconds=5 * 60 + 5))
            event.refresh_from_db()
            self.assertEqual(event.created_display(), "5 minutes ago")
            self.move_clock_on(timedelta(seconds=60 * 60))
            event.refresh_from_db()
            self.assertEqual(event.created_display(), "1 hour ago")
            self.move_clock_on(timedelta(seconds=9 * 60 * 60))
            event.refresh_from_db()
            self.assertEqual(event.created_display(), "10 hours ago")
            self.move_clock_on(timedelta(seconds=24 * 60 * 60))
            event.refresh_from_db()
            self.assertEqual(event.created_display(), "1 day ago")
            self.move_clock_on(timedelta(seconds=24 * 60 * 60))
            event.refresh_from_db()
            self.assertEqual(event.created_display(), "2 days ago")
