...
FILTER_LANGUAGES = [(FILTER_LANGUAGES_ALL, t_lazy("site-languages-all"))] + settings.LANGUAGES
...
class FilterFormMixin:
    ...
    def from_request_data(cls, request_data, defaults=None):
        ...
        if not instance.is_valid():
            raise AssertionError(f"{cls} should always be valid: {instance.errors.as_data()}")
        return instance
...
class SignUpForm(forms.ModelForm):
    ...
    username = forms.RegexField(
        ...
        max_length=40,
        regex=r"^[\w.+-]+$",
        error_messages={"invalid": t_lazy("accounts-username-validation")},
    )
    ...
    def clean_password(self):
        ...
        if len(p) < settings.MINIMUM_PASSWORD_LENGTH:
            raise forms.ValidationError(
                t("accounts-password-length-validation", dict(length=settings.MINIMUM_PASSWORD_LENGTH))
            )
        return p
...
class LogInForm(forms.Form):
    email = forms.CharField(label=t_lazy("accounts-login-email-or-username"), max_length=255)
    password = forms.CharField(
        label=t_lazy("accounts-password"), max_length=100, required=False, widget=forms.PasswordInput
    )
...
class AccountPasswordChangeForm(PasswordChangeForm):
    ...
    error_messages = dict(
        AccountSetPasswordForm.error_messages,
        **{
...
class VerseSetSearchForm(FilterFormMixin, forms.Form):
    ...
    set_type = forms.ChoiceField(
        choices=VERSE_SET_TYPE_CHOICES,
    ...
    order = forms.ChoiceField(
        choices=[
            (VERSE_SET_ORDER_POPULARITY, t_lazy("versesets-order-most-popular-first")),
    ...
    language_code = forms.ChoiceField(
        choices=FILTER_LANGUAGES, initial=settings.LANGUAGE_CODE, label=t_lazy("versesets-language")
    )
...
class LeaderboardFilterForm(FilterFormMixin, forms.Form):
    when = forms.ChoiceField(
        choices=[
            (LEADERBOARD_WHEN_ALL_TIME, t_lazy("leaderboards-filter-all-time")),
...
class GroupWallFilterForm(FilterFormMixin, forms.Form):
    order = forms.ChoiceField(
        choices=[
            (GROUP_WALL_ORDER_NEWEST_FIRST, t_lazy("groups-messages-order-most-recent-first")),
...
class GroupFilterForm(FilterFormMixin, forms.Form):
    ...
    language_code = forms.ChoiceField(
        choices=FILTER_LANGUAGES, label=t_lazy("groups-language"), help_text=t_lazy("groups-language.help-text")
    )
...
class UserVersesFilterForm(FilterFormMixin, forms.Form):
    query = forms.CharField(
        label=t_lazy("user-verses-filter-query"),
        required=False,
    ...
    order = forms.ChoiceField(
        choices=[
            (USER_VERSES_ORDER_WEAKEST, t_lazy("user-verses-order-weakest-first")),
