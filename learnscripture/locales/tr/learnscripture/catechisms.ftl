## Catechisms page.

# Page title
catechisms-page-title = İlmihal

# Introduction to catechisms on the catechism page
catechisms-page-intro =
    İlmihal: Hristiyanlığın temel öğretilerini soru-cevap halinde sunan bir edebi tarzdır.

    Protestan reform döneminde, din bilginleri Kutsal Kitap'ın temel gerçeklerini açıklamak için eski çağlardan kalma bir yöntemi kullanarak ilmihalleri oluşturdular. Bunlar günümüze dek yaygın kullanımda bulunmaktadır.

catechisms-choose-a-catechism-html =
    Çalışmak istediğiniz ilmihali seçiniz. Eklememizi istediğiniz bir ilmihal
    varsa, <a href="{ $contact_url }">bizimle irtibata geçebilirsiniz</a>.

# Button to start learning a catechism
catechisms-learn-button = Çalış

# Displayed at the bottom of a catechism description
catechisms-includes-qa-pairs-count = Bu ilmihalde { $count } adet soru-cevap bulunur.

## Individual catechism page.

# Sub-title for the description of the catechism
catechisms-description-title = Tanım

# Sub-title for the list of questions in the catechism
catechisms-questions-title = Sorular

# Sub-title for the section with notes about catechism learning
catechisms-notes-title = Notlar

# Instructions about how to stop learning a catechism
catechisms-how-to-opt-out = İlmihali çalışırken, bir soruyu atlamak için çalışma sayfasındaki
      ‘{ learn-stop-learning-this-question }’ düğmesine basınız.

# Note about learning catechisms being a public action
catechisms-learning-is-public = İlmihal çalışma eylemi kamu oyuna açıktır

# Link to page that lists all catechisms
catechisms-all-catechism-link = Tüm ilmihaller

# Sub-title for list of people learning the catechism
catechisms-people-learning-this-title = İlmihali çalışanlar
